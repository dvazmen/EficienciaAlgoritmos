
#include <iostream>
#include <algorithm>
#include <iterator>
#include <vector>
#include <chrono>
#include <fstream>
#include <cstring>


void bubbleSort( int v[], int N);
void cocktailSort(int v[], int N);
void insertionSort(int v[], int N);
void bucketSort(int v[], const int N);
//Shell
void shellSort(int v[], int N);
//

//Selection
void selectionSort(int v[], int N);
//

//Quick
void quickSort(int v[], int primero, int ultimo);
//

//Counting
void findMinMax(int v[], int N, int& mi, int& max);
void countingSort( int v[], int N);
//

//Merge
void merge(int v[], int l, int m, int n, int N);
void mergeSort(int v[], int l, int n, int N);
//


//Radix
int getMax(int v[], int N);
void countSort(int v[], int N, int exp);
void radixSort(int v[], int N);
//

//Binary Tree
struct Node {
    int val;
    Node* left;
    Node* right;

    Node(int n) {
        val = n;
        left = right = nullptr;
    }

    static void destroy(Node* node) {
        if (node) {
            destroy(node->left);
            destroy(node->right);
            delete node;
        }
    }
};
void insert(Node*& node, int val);
void inOrder(Node* node, std::vector<int>& a);
void binarySort(int a[], int N);
//

//Heap
void shift_down(int heap[],int i, int max);
void to_heap(int arr[], int N);
void heap_sort(int arr[], int N);
//

//Funciones main
void procesa(int);
void importa_numeros(int enteros[],int n){
    std::ifstream file("elementos.txt");
    if(file.is_open())
    {
        for(int i = 0; i < n; ++i)
        {
            file >> enteros[i];
        }

    }
    else if(!file.is_open())
    {
        std::cout<< "No lo pude cargar" << std::endl;
    }
}
//

int main()
{
    int cantidad[6] = {10,100,1000,10000,100000,1000000};
    for(int x = 0;x < 6;x++){
        procesa(cantidad[x]);
    }
    return 0;
}

void procesa(int n){
    int enteros[n];
    importa_numeros(enteros, n);
    std::cout << n << " numeros:" << std::endl;



    auto begin = std::chrono::high_resolution_clock::now();
    bucketSort(enteros, n);
    auto end = std::chrono::high_resolution_clock::now();
    auto bucket = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Bucket Sort: "<< bucket.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    countingSort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto counting = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Counting Sort: "<< counting.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    mergeSort(enteros, 0, n-1, n);
    end = std::chrono::high_resolution_clock::now();
    auto merge = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Merge Sort: " << merge.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);


    begin = std::chrono::high_resolution_clock::now();
    radixSort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto radix = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Radix Sort: "<< radix.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    selectionSort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto selection = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Selection Sort: "<< selection.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    binarySort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto binary = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Binary Tree Sort: "<< binary.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);


    begin = std::chrono::high_resolution_clock::now();
    shellSort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto shell = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Shell Sort: "<< shell.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    heap_sort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto heap = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Heap Sort: "<< heap.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    bubbleSort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto bubble = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Bubble Sort: "<< bubble.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);


    begin = std::chrono::high_resolution_clock::now();
    quickSort(enteros, 0, n-1);
    end = std::chrono::high_resolution_clock::now();
    auto quick = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Quick Sort: " << quick.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    insertionSort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto insertion = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Insertion Sort: "<< insertion.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);

    begin = std::chrono::high_resolution_clock::now();
    cocktailSort(enteros, n);
    end = std::chrono::high_resolution_clock::now();
    auto cocktail = std::chrono::duration_cast<std::chrono::microseconds>(end-begin);
    std::cout << "Cocktail Sort: "<< cocktail.count() << " ms"<< std::endl;
    importa_numeros(enteros,n);


    std::cout << std::endl;


}

void bubbleSort( int v[], int N){

    int temp;

    for (int i = 0; i < N; i++){
        for(int j= N-1; j>i; j--){
            if (v[j-1] > v[j])
            {
                temp = v[j-1];
                v[j-1] = v[j];
                v[j] = temp;
            }
        }
    }
}

void cocktailSort(int v[], int N){
    int lo = 0, hi = (N-1), mov = lo, temp;

    while (lo<hi){
        for(int i=hi; i>lo; --i){
            if(v[i-1]>v[i]){
                temp = v[i-1];
                v[i-1]=v[i];
                v[i]= temp;
                mov=i;
            }
        }

        lo = mov;

        for(int i=lo; i<hi; ++i){
            if(v[i] > v[i+1]){
                temp = v[i];
                v[i] = v[i+1];
                v[i+1]= temp;
                mov = i;
            }
        }
        hi=mov;
    }
}

void insertionSort(int v[], int N){

    int index;

    for (int i=1; i < N; i++)
    {
        index = v[i];

        int j = i-1;

        while (j >= 0 && v[j] > index)
        {
            v[j + 1] = v[j];
            j--;
        }

        v[j+1] = index;
    }

}

void bucketSort(int v[], const int N){

    int m = 101;
    int buckets[m];

    for(int i=0; i< m; ++i){
        buckets[i]=0;
    }

    for (int i = 0; i < N; ++i){
        ++buckets[v[i]];
    }

    for (int i =0, j = 0; j < m; ++j){
        for (int k = buckets[j]; k > 0; --k){
            v[i++] = j;
        }
    }


}


void merge(int v[], int l, int m, int n, int N)
{
    int i, j, k;
    int aux[N];

    for(i=m+1;i>l;i--)
        aux[i-1] = v[i-1];

    for (j=m; j<n; j++)
        aux[n+m-j] = v[j+1];

    for(k=l;k<=n;k++)
        if (!(aux[i]> aux[j]))
            v[k] = aux[i++];
        else
            v[k] = aux[j--];
}

void mergeSort(int v[], int l, int n, int N){
    int m = (n+l)/2;
    if (n > l)
    {
        mergeSort (v, l, m, N);
        mergeSort (v, m+1, n, N);
        merge (v, l, m, n, N);
    }
}


void insert(Node*& node, int val)
{
    if (!node)
        node = new Node(val);
    else if (val < node->val)
        insert(node->left, val);
    else
        insert(node->right, val);
}

void inOrder(Node* node, std::vector<int>& a)
{
    if (node) {
        inOrder(node->left, a);
        a.push_back(node->val);
        inOrder(node->right, a);
    }
}

void binarySort(int a[], int N)
{
    Node* root = nullptr;
    for (int i = 0; i < N; ++i)
        insert(root, a[i]);

    std::vector<int> aux;
    inOrder(root, aux);

    Node::destroy(root);

    for (int i = 0; i < aux.size(); ++i)
        a[i] = aux[i];
}

int getMax(int v[], int N){

    int max = v[0];

    for(int i=1; i<N; i++){
        if(v[i]>max){
            max = v[i];
        }
    }

    return max;
}

void countSort(int v[], int N, int exp){
    int output[N];
    int count[10] = {0};


    for(int i=0; i<N; i++){
        count[(v[i]/exp)%10]++;
    }

    for(int i=1; i<10;i++){
        count[i]+=count[i-1];
    }

    for (int i = N-1; i>=0; i--){

        output[count[( v[i]/exp ) %10 ] -1] = v[i];
        count[(v[i]/exp)%10]--;
    }

    for(int i=0;i<N; i++){
        v[i]=output[i];
    }
}

void radixSort(int v[], int N){
    int m = getMax(v, N);

    for(int exp = 1; m/exp>0; exp*=10){
        countSort(v, N, exp);
    }
}

void shellSort(int v[], int N){
    int temp;

    for(int gap= N/2; gap>0; gap/=2){
        for(int i=gap; i<N; i++){
            for(int j=i-gap; j>=0 && v[j]>v[j+gap]; j-=gap){
                temp = v[j];
                v[j]= v[j+gap];
                v[j+gap] = temp;
            }
        }
    }
}


void selectionSort(int v[], int N){
    int minimo = 0;
    int temp;

    for(int i = 0; i < N-1; i++){
        minimo = i;
        for(int j = i + 1; j < N; j++){
            if (v[minimo] > v[j])
                minimo = j;
        }
        temp = v[minimo];
        v[minimo] = v[i];
        v[i] = temp;
    }
}

void quickSort(int v[], int primero, int ultimo){
    int izquierdo = primero;
    int derecho = ultimo;
    int temp;
    int pivote = v[primero];

    if ( primero < ultimo)
    {
        while ( izquierdo < derecho)
        {
            while ( (izquierdo < derecho) && !(v[izquierdo] > pivote)) izquierdo++;
            while (v[derecho]> pivote ) derecho--;

            if ( izquierdo < derecho)
            {
                temp = v[derecho];
                v[derecho] = v[izquierdo];
                v[izquierdo] = temp;
            }
        }
        temp = v[derecho];
        v[derecho] = v[primero];
        v[primero] = temp;
        quickSort ( v, primero, derecho-1);
        quickSort ( v, izquierdo, ultimo);
    }
}

void findMinMax(int v[], int N, int& mi, int& max){
    mi = 100;
    max = 0;

    for( int i = 0; i < N; i++ ){
        if( v[i] > max ) max = v[i];
        if( v[i] < mi ) mi = v[i];
    }

}


void countingSort( int v[], int N){
    int mi, max;
    int z=0;
    findMinMax(v, N, mi, max);
    int nlen = (max-mi)+1;
    int* temp = new int[nlen];
    memset(temp, 0, nlen * sizeof( int ));

    for( int i = 0; i < N; i++ ){
        temp[v[i] - mi]++;
    }
    for( int i = mi; i <= max; i++ ){
        while( temp[i - mi] )
        {
            v[z++] = i;
            temp[i - mi]--;
        }
    }
    delete [] temp;
}

void shift_down(int heap[],int i, int max) {
    int i_big, c1, c2;
    while(i < max) {
        i_big = i;
        c1 = (2*i) + 1;
        c2 = c1 + 1;
        if( c1<max && heap[c1]>heap[i_big] )
            i_big = c1;
        if( c2<max && heap[c2]>heap[i_big] )
            i_big = c2;
        if(i_big == i) return;
        std::swap(heap[i],heap[i_big]);
        i = i_big;
    }
}

void to_heap(int v[], int N) {
    int i = (N/2) - 1;
    while(i >= 0) {
        shift_down(v, i, N);
        --i;
    }
}

void heap_sort(int v[], int N) {
    to_heap(v, N);
    int end = N - 1;
    while (end > 0) {
        std::swap(v[0], v[end]);
        shift_down(v, 0, end);
        --end;
    }
}


